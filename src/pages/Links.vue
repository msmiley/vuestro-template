<template>
  <vuestro-container no-wrap column overflow-hidden>
    <vuestro-card cols="6" color="var(--vuestro-gold)" overflow-hidden>
      ...
      <vuestro-container frame gutter="none">
        <vuestro-force-graph :data="forceGraphData" :options="graphOptions" @select="onSelectSpoke"></vuestro-force-graph>
      </vuestro-container>
    </vuestro-card>
    <vuestro-card cols="6" overflow-hidden>
      <vuestro-panel stretch scroll>
        ...
        <template #toolbar>
				...
				<div v-if="Object.keys(selectedSpoke).length == 0" class="no-data">Select a node to see its current state</div>
